...
function loadFromUrl(){title.textContent='Loading character…';detail.textContent='Requesting ./moe-beginner.vrm';loader.load('./moe-beginner.vrm?v=6a1',gltf=>{try{useGLTF(gltf,'Loaded from GitHub Pages');}catch(e){showError(e);}},p=>{if(p.total)detail.textContent=`Downloading character… ${Math.round(p.loaded/p.total*100)}%`;else detail.textContent=`Downloading character… ${Math.round(p.loaded/1024/1024)} MB`;},showError);}
...
function showRewardToast(xp,coins,label='Reward earned'){const toast=document.getElementById('rewardToast');toast.innerHTML=`⭐ +${xp} XP &nbsp; 🪙 +${coins}<br><small>${label}</small>`;toast.classList.remove('show');void toast.offsetWidth;toast.classList.add('show');clearTimeout(showRewardToast.timer);showRewardToast.timer=setTimeout(()=>toast.classList.remove('show'),2200);}
function showLevelUp(level){document.getElementById('levelUpNumber').textContent=level;document.getElementById('levelUpMessage').textContent=level>=10?'Amazing progress — keep going!':'Your English adventure is getting stronger!';const back=document.getElementById('levelUpBackdrop');back.classList.add('open');back.setAttribute('aria-hidden','false');}
function grantReward({xp=0,coins=0,label='Activity complete'}={}){xp=Math.max(0,Math.round(Number(xp)||0));coins=Math.max(0,Math.round(Number(coins)||0));const oldLevel=progress.level;progress.xp+=xp;progress.totalXp+=xp;progress.coins+=coins;normalizeProgress();const newLevel=progress.level;renderProgress();showRewardToast(xp,coins,label);if(newLevel>oldLevel)setTimeout(()=>showLevelUp(newLevel),500);return {level:newLevel,xp:progress.xp,coins:progress.coins,leveledUp:newLevel>oldLevel};}
window.MoeRewards={grant:grantReward,getProgress:()=>({...progress})};
...
document.getElementById('testPractice').onclick=()=>grantReward({xp:25,coins:10,label:'Practice complete!'});
document.getElementById('testQuiz').onclick=()=>grantReward({xp:60,coins:30,label:'Great quiz!'});
document.getElementById('testBoss').onclick=()=>grantReward({xp:120,coins:75,label:'Boss stage cleared!'});
document.getElementById('closeLevelUp').onclick=()=>{const b=document.getElementById('levelUpBackdrop');b.classList.remove('open');b.setAttribute('aria-hidden','true');};
