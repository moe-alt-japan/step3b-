...
function loadFromUrl(){title.textContent='Loading character…';detail.textContent='Requesting ./moe-beginner.vrm';loader.load('./moe-beginner.vrm?v=4a4',gltf=>{try{useGLTF(gltf,'Loaded from GitHub Pages');}catch(e){showError(e);}},p=>{if(p.total)detail.textContent=`Downloading character… ${Math.round(p.loaded/p.total*100)}%`;else detail.textContent=`Downloading character… ${Math.round(p.loaded/1024/1024)} MB`;},showError);}
...
let visibleAccessory=null;
function updateVisibleAccessory(){
  if(visibleAccessory){visibleAccessory.removeFromParent();visibleAccessory=null;}
  if(!vrm||progress.equipped.accessory!=='headphones')return;
  const head=vrm.humanoid?.getNormalizedBoneNode?.('head')||vrm.humanoid?.getRawBoneNode?.('head');
  if(!head)return;
  const group=new THREE.Group();
  const mat=new THREE.MeshStandardMaterial({color:0x3b82f6,roughness:0.45});
  const band=new THREE.Mesh(new THREE.TorusGeometry(0.13,0.014,8,32,Math.PI),mat);
  band.rotation.z=Math.PI; band.position.set(0,0.08,0); group.add(band);
  const geo=new THREE.BoxGeometry(0.035,0.075,0.04);
  const left=new THREE.Mesh(geo,mat),right=new THREE.Mesh(geo,mat);
  left.position.set(-0.135,0.01,0.01);right.position.set(0.135,0.01,0.01);group.add(left,right);
  head.add(group);visibleAccessory=group;
}

function renderProgress(){normalizeProgress();const need=xpNeeded(progress.level);document.getElementById('level').textContent=progress.level;document.getElementById('xp').textContent=progress.xp;document.getElementById('xpNeed').textContent=need;document.getElementById('xpFill').style.width=`${Math.min(100,progress.xp/need*100)}%`;document.getElementById('headerCoins').textContent=progress.coins.toLocaleString();document.getElementById('coinStat').textContent=progress.coins.toLocaleString();document.getElementById('totalXp').textContent=progress.totalXp.toLocaleString();document.getElementById('shopCoins').textContent=progress.coins.toLocaleString();document.getElementById('ownedCount').textContent=progress.owned.length;applyBackground();updateVisibleAccessory();saveProgress();renderShop();renderInventory();}
